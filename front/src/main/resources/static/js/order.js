						...
						$.each(item.orderDetails, function(i, detail){
							html += '<tr>\
										<td class="goods">\
											<div class="goods-box">\
												<div class="img">\
													<a href="/getGoodsInfo/'+detail.goods.id+'"><img src="/upload/'+detail.goods.pics[0].url+'" ></a>\
												</div>\
												<div class="goods-info">\
													<p class="name"><a href="/getGoodsInfo/'+detail.goods.id+'">'+detail.goods.name+' '+detail.goods.version+' '+detail.goods.color+'</a></p>\
										...
										<td class="actions">'+
											...
											(  item.status == 1 ||item.status == 0 ? '<a class="" style="background: #1e9fff;color: #F25807"  href="/user/order/cancel?orderId='+item.id+'">取消订单</a>' : '')+
											(item.status==3 ? '<a class="" style="background: #dee06e;color: #F25807"  href="/user/order/updateStatus?orderId='+item.id+'&status=5' + ''+' ">退货</a>' : '')+
											...
											( item.status != 8 ? '<a class="" href="/user/order/updateStatus?orderId='+item.id+'&status=8' + ''+' ">删除订单</a>' : '')+
